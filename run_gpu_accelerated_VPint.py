...
from VPint.cupy_WP_MRP import WP_SMRP
...
scenes.append(["europe_cropland_hungary", target_europe_cropland_hungary, m1_europe_cropland_hungary, mask_europe_cropland_hungary])
...
scenes.append(["europe_cropland_ukraine", target_europe_cropland_ukraine, m1_europe_cropland_ukraine, mask_europe_cropland_ukraine])
...
scenes.append(["america_shrubs_mexico", target_america_shrubs_mexico, m1_america_shrubs_mexico, mask_america_shrubs_mexico])
...
scenes.append(["asia_herbaceous_mongoliaeast", target_asia_herbaceous_mongoliaeast, m1_asia_herbaceous_mongoliaeast, mask_asia_herbaceous_mongoliaeast])
...
scenes.append(["africa_herbaceous_southafrica", target_africa_herbaceous_southafrica, m1_africa_herbaceous_southafrica, mask_africa_herbaceous_southafrica])
...
scenes.append(["australia_shrubs_south", target_australia_shrubs_south, m1_australia_shrubs_south, mask_australia_shrubs_south])
...
scenes.append(["america_cropland_iowa", target_america_cropland_iowa, m1_america_cropland_iowa, mask_america_cropland_iowa])
...
scenes.append(["asia_herbaceous_kazakhstan", target_asia_herbaceous_kazakhstan, m1_asia_herbaceous_kazakhstan, mask_asia_herbaceous_kazakhstan])
...
scenes.append(["america_herbaceous_peru", target_america_herbaceous_peru, m1_america_herbaceous_peru, mask_america_herbaceous_peru])
...
scenes.append(["asia_shrubs_indiapakistan", target_asia_shrubs_indiapakistan, m1_asia_shrubs_indiapakistan, mask_asia_shrubs_indiapakistan])
...
scenes.append(["australia_shrubs_west", target_australia_shrubs_west, m1_australia_shrubs_west, mask_australia_shrubs_west])
...
scenes.append(["america_urban_atlanta", target_america_urban_atlanta, m1_america_urban_atlanta, mask_america_urban_atlanta])
...
scenes.append(["america_forest_mississippi", target_america_forest_mississippi, m1_america_forest_mississippi, mask_america_forest_mississippi])
...
def run_patch(base_path, target_n, m1_n, mask_n, y_size, x_size, y_offset, x_offset, counter, buffer_mask=False,
              cloud_threshold=20, method="exact", mask_buffer_size=5):
    print("Current patch # is:", counter)
    target_path = base_path + "/" + target_n + ".zip"
    feature_path = base_path + "/" + m1_n + ".zip"
    mask_path = base_path + "/" + mask_n + ".zip"
    ...
    if (not (np.any(target > 0))):
        ...
        print("\n")
        return_list = [np.zeros(target.shape), "no clouds", "no clouds", "no clouds", "no clouds"]
        return return_list
    ...
    if (not (np.any(mask > cloud_threshold))):
        return_list = [target, "no clouds", "no clouds", "no clouds", "no clouds"]
        return return_list
    ...
    # Start VPint with CuPy arrays
    start_vpint = time.time()
    pred_grid0 = VPint_interpolation(cp.array(target_cloudy), cp.array(features))
    end_vpint = time.time()
    vpint_time = end_vpint - start_vpint

    pred_grid = cp.asnumpy(pred_grid0)

    start_metrics = time.time()
    if (pred_grid.shape != target.shape):
        print("Mismatched shapes for ", pd, ", True shape:", target.shape, ", pred shape:", pred_grid.shape)
        ...
        pred_mean = np.nanmean(pred_grid)
        true_mean = np.nanmean(target)
        pred_vals = np.nan_to_num(pred_grid, nan=pred_mean)
    ...
    return_list = [pred_grid, vpint_time, metrics_time, mae, mse]
...
for scene in scenes:
    ...
    scene_metrics = [["patch", "patch runtime", "VPint runtime", "Metrics runtime", "MAE", "MSE"]]
    # Iterate
    for y_offset in row_list:
        for x_offset in col_list:
            ...
            patch_end = time.time()
            scene_metrics.append([patch_name, patch_end - patch_start, outp[1], outp[2], outp[3], outp[4]])

    scene_end = time.time()
    scene_metrics.append(["Entire scene", scene_end - scene_start, "nan", "nan", "nan", "nan"])
    scene_name = scene_dir + "/" + scene[0] + "_gpu_results.csv"
